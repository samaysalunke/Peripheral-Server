...
static void WIFI_ScbModeStop(void)
{
    ...
    *******************************************************************************/
    void WIFI_SetPins(uint32 mode, uint32 subMode, uint32 uartEnableMask)
    {
        ...
        {
            WIFI_RX_SDA_MOSI_HSIOM_SEL_GPIO,
            WIFI_TX_SCL_MISO_HSIOM_SEL_GPIO,
            0u,
            0u,
            0u,
            0u,
            ...
            (WIFI_SCB_MODE_EZI2C == mode))
        {
            ...
            hsiomSel[WIFI_RX_SDA_MOSI_PIN_INDEX] = WIFI_RX_SDA_MOSI_HSIOM_SEL_I2C;
            pinsDm  [WIFI_RX_SDA_MOSI_PIN_INDEX] = WIFI_PIN_DM_OD_LO;
        #elif (WIFI_RX_WAKE_SDA_MOSI_PIN)
            hsiomSel[WIFI_RX_WAKE_SDA_MOSI_PIN_INDEX] = WIFI_RX_WAKE_SDA_MOSI_HSIOM_SEL_I2C;
            pinsDm  [WIFI_RX_WAKE_SDA_MOSI_PIN_INDEX] = WIFI_PIN_DM_OD_LO;
        #else
        #endif /* (WIFI_RX_SDA_MOSI_PIN) */
        
        #if (WIFI_TX_SCL_MISO_PIN)
            hsiomSel[WIFI_TX_SCL_MISO_PIN_INDEX] = WIFI_TX_SCL_MISO_HSIOM_SEL_I2C;
            pinsDm  [WIFI_TX_SCL_MISO_PIN_INDEX] = WIFI_PIN_DM_OD_LO;
        ...
        {
        #if (WIFI_RX_SDA_MOSI_PIN)
            hsiomSel[WIFI_RX_SDA_MOSI_PIN_INDEX] = WIFI_RX_SDA_MOSI_HSIOM_SEL_SPI;
        #elif (WIFI_RX_WAKE_SDA_MOSI_PIN)
            hsiomSel[WIFI_RX_WAKE_SDA_MOSI_PIN_INDEX] = WIFI_RX_WAKE_SDA_MOSI_HSIOM_SEL_SPI;
        #else
        #endif /* (WIFI_RX_SDA_MOSI_PIN) */
        
        #if (WIFI_TX_SCL_MISO_PIN)
            hsiomSel[WIFI_TX_SCL_MISO_PIN_INDEX] = WIFI_TX_SCL_MISO_HSIOM_SEL_SPI;
        #endif /* (WIFI_TX_SCL_MISO_PIN) */
            ...
            {
                /* Slave */
                pinsDm[WIFI_RX_SDA_MOSI_PIN_INDEX] = WIFI_PIN_DM_DIG_HIZ;
                pinsDm[WIFI_TX_SCL_MISO_PIN_INDEX] = WIFI_PIN_DM_STRONG;
                ...
                /* Disable input buffer */
                 pinsInBuf |= WIFI_TX_SCL_MISO_PIN_MASK;
            #endif /* (WIFI_TX_SCL_MISO_PIN) */
            }
            else 
            {
                /* (Master) */
                pinsDm[WIFI_RX_SDA_MOSI_PIN_INDEX] = WIFI_PIN_DM_STRONG;
                pinsDm[WIFI_TX_SCL_MISO_PIN_INDEX] = WIFI_PIN_DM_DIG_HIZ;
            ...
            #if (WIFI_RX_SDA_MOSI_PIN)
                pinsInBuf |= WIFI_RX_SDA_MOSI_PIN_MASK;
            #elif (WIFI_RX_WAKE_SDA_MOSI_PIN)
                pinsInBuf |= WIFI_RX_WAKE_SDA_MOSI_PIN_MASK;
            #else
            #endif /* (WIFI_RX_SDA_MOSI_PIN) */
        ...
        else /* UART */
        {
            if (WIFI_UART_MODE_SMARTCARD == subMode)
            {
                /* SmartCard */
            #if (WIFI_TX_SCL_MISO_PIN)
                hsiomSel[WIFI_TX_SCL_MISO_PIN_INDEX] = WIFI_TX_SCL_MISO_HSIOM_SEL_UART;
                pinsDm  [WIFI_TX_SCL_MISO_PIN_INDEX] = WIFI_PIN_DM_OD_LO;
            #endif /* (WIFI_TX_SCL_MISO_PIN) */
            }
            else /* Standard or IrDA */
            {
                ...
                {
                #if (WIFI_RX_SDA_MOSI_PIN)
                    hsiomSel[WIFI_RX_SDA_MOSI_PIN_INDEX] = WIFI_RX_SDA_MOSI_HSIOM_SEL_UART;
                    pinsDm  [WIFI_RX_SDA_MOSI_PIN_INDEX] = WIFI_PIN_DM_DIG_HIZ;
                #elif (WIFI_RX_WAKE_SDA_MOSI_PIN)
                    hsiomSel[WIFI_RX_WAKE_SDA_MOSI_PIN_INDEX] = WIFI_RX_WAKE_SDA_MOSI_HSIOM_SEL_UART;
                    pinsDm  [WIFI_RX_WAKE_SDA_MOSI_PIN_INDEX] = WIFI_PIN_DM_DIG_HIZ;
                #else
                #endif /* (WIFI_RX_SDA_MOSI_PIN) */
                }
    ...
    /* Note: the DR register settings do not effect the pin output if HSIOM is other than GPIO */
        ...
        WIFI_SET_HSIOM_SEL(WIFI_RX_SDA_MOSI_HSIOM_REG,
                                       ...
                                       WIFI_RX_SDA_MOSI_HSIOM_POS,
                                        hsiomSel[WIFI_RX_SDA_MOSI_PIN_INDEX]);

        WIFI_uart_rx_i2c_sda_spi_mosi_SetDriveMode((uint8) pinsDm[WIFI_RX_SDA_MOSI_PIN_INDEX]);

        #if (!WIFI_CY_SCBIP_V1)
            WIFI_SET_INP_DIS(WIFI_uart_rx_i2c_sda_spi_mosi_INP_DIS,
                                         WIFI_uart_rx_i2c_sda_spi_mosi_MASK,
                                         (0u != (pinsInBuf & WIFI_RX_SDA_MOSI_PIN_MASK)));
        #endif /* (!WIFI_CY_SCBIP_V1) */
        ...
        WIFI_SET_HSIOM_SEL(WIFI_RX_WAKE_SDA_MOSI_HSIOM_REG,
                                       ...
                                       hsiomSel[WIFI_RX_WAKE_SDA_MOSI_PIN_INDEX]);

        WIFI_uart_rx_wake_i2c_sda_spi_mosi_SetDriveMode((uint8)
                                                               pinsDm[WIFI_RX_WAKE_SDA_MOSI_PIN_INDEX]);

        WIFI_SET_INP_DIS(WIFI_uart_rx_wake_i2c_sda_spi_mosi_INP_DIS,
                                     WIFI_uart_rx_wake_i2c_sda_spi_mosi_MASK,
                                     (0u != (pinsInBuf & WIFI_RX_WAKE_SDA_MOSI_PIN_MASK)));

         /* Set interrupt on falling edge */
        WIFI_SET_INCFG_TYPE(WIFI_RX_WAKE_SDA_MOSI_INTCFG_REG,
                                        WIFI_RX_WAKE_SDA_MOSI_INTCFG_TYPE_MASK,
    ...
    #if (WIFI_TX_SCL_MISO_PIN)
        WIFI_SET_HSIOM_SEL(WIFI_TX_SCL_MISO_HSIOM_REG,
                                       WIFI_TX_SCL_MISO_HSIOM_MASK,
                                       WIFI_TX_SCL_MISO_HSIOM_POS,
                                        hsiomSel[WIFI_TX_SCL_MISO_PIN_INDEX]);

        WIFI_uart_tx_i2c_scl_spi_miso_SetDriveMode((uint8) pinsDm[WIFI_TX_SCL_MISO_PIN_INDEX]);

    #if (!WIFI_CY_SCBIP_V1)
        WIFI_SET_INP_DIS(WIFI_uart_tx_i2c_scl_spi_miso_INP_DIS,
                                     WIFI_uart_tx_i2c_scl_spi_miso_MASK,
                                    (0u != (pinsInBuf & WIFI_TX_SCL_MISO_PIN_MASK)));
    #endif /* (!WIFI_CY_SCBIP_V1) */
    #endif /* (WIFI_RX_SDA_MOSI_PIN) */
