    ...
    *******************************************************************************/
    void WIFI_UartInit(const WIFI_UART_INIT_STRUCT *config)
    {
        ...
        else
        {
            ...
            WIFI_rxBufferSize  = (uint8) config->rxBufferSize;

            /* Set TX direction internal variables */
            WIFI_txBuffer      =         config->txBuffer;
            WIFI_txDataBits    = (uint8) config->dataBits;
            WIFI_txBufferSize  = (uint8) config->txBufferSize;
...
void WIFI_UartPostEnable(void)
{
        ...
        if (WIFI_CHECK_TX_SCL_MISO_PIN_USED)
        {
            /* Set SCB UART to drive the output pin */
            WIFI_SET_HSIOM_SEL(WIFI_TX_SCL_MISO_HSIOM_REG, WIFI_TX_SCL_MISO_HSIOM_MASK,
                                           WIFI_TX_SCL_MISO_HSIOM_POS, WIFI_TX_SCL_MISO_HSIOM_SEL_UART);
        }
    #endif /* (WIFI_TX_SCL_MISO_PIN_PIN) */
...
void WIFI_UartStop(void)
{
        ...
        if (WIFI_CHECK_TX_SCL_MISO_PIN_USED)
        {
            ...
            WIFI_SET_HSIOM_SEL(WIFI_TX_SCL_MISO_HSIOM_REG, WIFI_TX_SCL_MISO_HSIOM_MASK,
                                           WIFI_TX_SCL_MISO_HSIOM_POS, WIFI_TX_SCL_MISO_HSIOM_SEL_GPIO);
        }
    #endif /* (WIFI_TX_SCL_MISO_PIN_PIN) */
